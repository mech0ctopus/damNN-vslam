...
def _batchGenerator(X_filelist,y_filelist,batchSize):
    ...
    while True:
        ...
        while idx+(batchSize-1)<len(X_filelist):
            X_train_1=np.zeros((batchSize,192,640),dtype=np.uint8)   #time=t, depth
            X_train_2=np.zeros((batchSize,192,640),dtype=np.uint8)   #time=t-1, depth
            ...
            for i in range(batchSize):
                #Load images
                X_train_1[i]=depth_read(y_filelist[idx+i])   #time=t, depth
                X_train_2[i]=depth_read(y_filelist[idx-1+i])   #time=t-1, depth
            ...
            X_train_1 = X_train_1.reshape(batchSize,640,192,1).astype(np.uint8)
            X_train_2 = X_train_2.reshape(batchSize,640,192,1).astype(np.uint8)
            
            #X_train_1 = X_train_1.reshape((X_train_1.shape[0],1,-1)).astype(np.uint8)
            #X_train_1 = X_train_1.squeeze()
            #X_train_2 = X_train_2.reshape((X_train_1.shape[0],1,-1)).astype(np.uint8)
            ...
            #X_train_1=X_train_1.reshape((batchSize,len(X_train_1)))
            #X_train_1=X_train_1.reshape((batchSize,X_train_1[0].size))
            #X_train_2=X_train_2.reshape((batchSize,len(X_train_2)))
            #X_train_2=X_train_2.reshape((batchSize,X_train_2[0].size))
...
def _valBatchGenerator(X_val_filelist,y_val_filelist,batchSize):
    ...
    while True:
        ...
        while idx+(batchSize-1)<len(X_val_filelist):
            X_val_1=np.zeros((batchSize,192,640),dtype=np.uint8)   #time=t, depth
            X_val_2=np.zeros((batchSize,192,640),dtype=np.uint8)   #time=t-1, depth
            ...
            for i in range(batchSize):
                #Load images
                X_val_1[i]=depth_read(y_val_filelist[idx+i])   #time=t, depth
                X_val_2[i]=depth_read(y_val_filelist[idx-1+i]) #time=t-1, depth
            ...
            #Reshape [samples][width][height][pixels]
            X_val_1 = X_val_1.reshape(batchSize,640,192,1).astype(np.uint8)
            X_val_2 = X_val_2.reshape(batchSize,640,192,1).astype(np.uint8)
            
            #X_val_1 = X_val_1.reshape((X_val_1.shape[0],1,-1)).astype(np.uint8)
            #X_val_1 = X_val_1.squeeze()
            #X_val_2 = X_val_2.reshape((X_val_1.shape[0],1,-1)).astype(np.uint8)
            #X_val_2 = X_val_2.squeeze()
            ...
            #X_val_1=X_val_1.reshape((batchSize,len(X_val_1)))
            #X_val_2=X_val_2.reshape((batchSize,len(X_val_2)))
            #X_val_1=X_val_1.reshape((batchSize,X_val_1[0].size))
            #X_val_2=X_val_2.reshape((batchSize,X_val_2[0].size))
